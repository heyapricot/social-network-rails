...
RSpec.describe User, type: :model do
  ...
  let!(:user) do
    FactoryBot.create(:user_with_posts, posts_count: post_quantity)
  end
  let!(:friendships) do
  ...
  describe 'friendships' do
    ...
    it 'can get a list of received friend requests' do
      expect(user.received_friend_requests).to match_array(
    ...
    it 'can get a friendship object that shares with another user' do
      expect(user.get_friendship(friendships.first.friend)).to eq(
        friendships.first
      )
    end
    ...
    context 'when accepting a friend request' do
      let(:friend_request) do
        FactoryBot.create(:friendship, user: user, status: :requested)
      end
